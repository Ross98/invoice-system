  ...
  <div>
    ...
    <v-card class="mb-4">
      <v-card-title class="d-flex align-center">
        ...
        <v-btn color="primary" variant="tonal" class="mr-2" @click="editInvoice">
    ...
    <template v-else>
      ...
      <v-card>
        <v-tabs v-model="activeTab" color="primary" class="px-2">
          <v-tab value="info">
            <v-icon left size="20">mdi-card-text</v-icon>
            基本信息
          </v-tab>
          <v-tab value="details">
            <v-icon left size="20">mdi-format-list-bulleted</v-icon>
            消费明细
            <v-chip v-if="invoice.details?.length" size="x-small" class="ml-1" color="primary" variant="tonal">
          ...
          <v-tab value="files">
            ...
            原文件
            <v-chip v-if="invoice.files?.length" size="x-small" class="ml-1" color="primary" variant="tonal">
          ...
          <v-tabs-window v-model="activeTab">
              ...
              <v-row>
                ...
                <v-col cols="12" md="8">
                  <v-card variant="outlined">
                    <v-card-text>
                      <div class="text-subtitle-1 font-weight-bold mb-4">发票信息</div>
                      <v-table density="compact">
                        <tbody>
                          <tr>
                            <td class="font-weight-bold text-caption" width="120">发票号码</td>
                            <td class="text-body-2">{{ invoice.invoice_number }}</td>
                            <td class="font-weight-bold text-caption" width="120">发票代码</td>
                            <td class="text-body-2">{{ invoice.invoice_code }}</td>
                          </tr>
                          <tr>
                            <td class="font-weight-bold text-caption">发票类型</td>
                            <td class="text-body-2">{{ invoice.invoice_type }}</td>
                            <td class="font-weight-bold text-caption">开票日期</td>
                            <td class="text-body-2">{{ formatDate(invoice.invoice_date) }}</td>
                          </tr>
                          <tr>
                            <td class="font-weight-bold text-caption">校验码</td>
                            <td class="text-body-2">{{ invoice.check_code || '无' }}</td>
                            <td class="font-weight-bold text-caption">对方单位</td>
                            <td class="text-body-2">{{ invoice.counterpart?.name || '未指定' }}</td>
                          </tr>
                          <tr>
                            <td class="font-weight-bold text-caption">消费分类</td>
                            <td class="text-body-2">{{ invoice.category?.name || '未分类' }}</td>
                            <td class="font-weight-bold text-caption">备注</td>
                            <td class="text-body-2">{{ invoice.remark || '无' }}</td>
                          </tr>
                        </tbody>
                      </v-table>
                    </v-card-text>
                ...
                <!-- 金额卡片 -->
                <v-col cols="12" md="4">
                  <v-card variant="outlined" class="text-center pa-4 mb-3">
                    <div class="text-caption text-grey">含税总金额</div>
                    <div class="text-h4 mt-2 text-primary">¥{{ formatAmount(invoice.total_with_tax) }}</div>
                  </v-card>
                  <v-card variant="outlined" class="text-center pa-4 mb-3">
                    <div class="text-caption text-grey">不含税金额</div>
                    <div class="text-h5 mt-1">¥{{ formatAmount(invoice.total_amount) }}</div>
                  </v-card>
                  <v-card variant="outlined" class="text-center pa-4">
                    <div class="text-caption text-grey">税额</div>
                    <div class="text-h5 mt-1">¥{{ formatAmount(invoice.tax_amount) }}</div>
                  </v-card>
                </v-col>
              </v-row>
            ...
            <v-tabs-window-item value="details">
              ...
              <div v-else class="text-center py-8">
                ...
                <div class="text-body-1 mt-3 text-grey">暂无消费明细</div>
              </div>
            </v-tabs-window-item>
            ...
            <v-tabs-window-item value="files">
                ...
                <v-list lines="two">
                  <v-list-item v-for="file in invoice.files" :key="file.id">
                    <template v-slot:prepend>
                      <v-icon :color="fileIconColor(file.file_type)" size="28">
                    ...
                    <template v-slot:append>
                      <v-btn icon variant="text" size="small" @click="previewFile(file)">
                        <v-icon>mdi-eye</v-icon>
                      </v-btn>
                      <v-btn icon variant="text" size="small" @click="downloadFile(file)">
                        <v-icon>mdi-download</v-icon>
                      </v-btn>
                      <v-btn icon variant="text" size="small" color="error" @click="deleteFile(file)">
              ...
              <div v-else class="text-center py-8">
                ...
                <div class="text-body-1 mt-3 text-grey">暂无发票文件</div>
                <div class="text-caption text-grey mt-1">点击下方按钮上传发票原文件</div>
              </div>

              <div class="d-flex justify-end mt-4">
                <v-btn color="primary" variant="tonal" @click="openUpload">
    ...
    <v-dialog v-model="uploadDialog" max-width="400">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" :loading="uploadingFile" @click="handleUpload">上传</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
