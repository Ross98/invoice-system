  ...
  <div>
    ...
    <v-row class="mb-4">
      <v-col cols="12" sm="4">
        <v-card border flat>
          <v-card-text class="pa-3 d-flex align-center">
            ...
            <div>
              <div class="text-caption text-medium-emphasis">筛选结果</div>
              <div class="text-h6 font-weight-bold">{{ invoices.length }} 张</div>
            </div>
          </v-card-text>
        </v-card>
      </v-col>
        ...
        <v-card border flat>
          <v-card-text class="pa-3 d-flex align-center">
            ...
            <div>
              ...
              <div class="text-h6 font-weight-bold">¥{{ fmtAmount(totalAmount) }}</div>
            </div>
          </v-card-text>
        </v-card>
      </v-col>
      <v-col cols="12" sm="4">
        <v-card border flat>
          <v-card-text class="pa-3 d-flex align-center">
            <v-avatar size="36" color="warning" variant="tonal" class="mr-3">
              <v-icon size="18">mdi-clock-outline</v-icon>
            </v-avatar>
            <div>
              <div class="text-caption text-medium-emphasis">待报销</div>
              <div class="text-h6 font-weight-bold">{{ pendingCount }} 张</div>
            </div>
          </v-card-text>
        </v-card>
      </v-col>
    ...
    <v-card class="mb-4" border flat>
      <v-card-text class="pb-0">
        <v-row>
          ...
          <v-col cols="6" sm="3" md="2">
            <v-btn color="primary" block variant="tonal" @click="loadInvoices">查询</v-btn>
          </v-col>
        </v-row>
      </v-card-text>
    </v-card>
    ...
    <!-- 分类占比预览条 -->
    <v-card class="mb-4" border flat v-if="categoryBars.length > 0">
      <v-card-title class="text-subtitle-2 font-weight-bold py-2">分类占比</v-card-title>
      <v-card-text class="py-0">
        <div class="category-bar-stack mb-2" style="height: 24px; border-radius: 12px; overflow: hidden; display: flex;">
          <div
        ...
        <div class="d-flex flex-wrap ga-3 pb-2">
          <div v-for="(bar, i) in categoryBars.slice(0, 8)" :key="bar.name" class="d-flex align-center">
            <div
              :style="{
    ...
    <!-- 操作栏 -->
    <v-card class="mb-4" border flat v-if="selectedIds.length > 0">
    ...
    <v-card border flat>
      ...
      >
        ...
        <template #item.category="{ item }">
          <v-chip size="x-small" variant="flat">{{ item.category?.name || '-' }}</v-chip>
        </template>
