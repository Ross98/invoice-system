<template>
  <div>
    <v-card class="mb-4">
      <v-card-title class="text-h5">上传发票文件</v-card-title>
      ...
      <v-col cols="12" md="6">
        <v-card>
          ...
          <v-card-text>
            ...
            <!-- OCR 选项 -->
            <div class="text-h6 mb-2">OCR 设置</div>
            <v-select
              v-model="ocrLang"
              label="识别语言"
              :items="ocrLanguages"
        ...
        <v-card>
          ...
          <v-card-text>
            <div v-if="uploading" class="text-center py-4">
              ...
              <div class="mt-2">正在上传和识别...</div>
            </div>

            <div v-else-if="parsedInvoices.length > 0">
              <v-expansion-panels>
                <v-expansion-panel v-for="(parsed, index) in parsedInvoices" :key="index">
                  ...
                  <v-expansion-panel-text>
                    <v-card variant="outlined" class="pa-4">
                      ...
                      <div class="text-body-2 font-weight-bold mb-2">解析结果：</div>
                      ...
                      <v-table density="compact">
                        <tbody>
                          ...
                          <tr>
                            <td class="font-weight-bold">发票代码</td>
                          ...
                          <tr>
                            <td class="font-weight-bold">金额</td>
                          ...
                          <tr>
                            <td class="font-weight-bold">销方单位</td>
            ...
            <div v-else class="text-center py-8">
              ...
              <div class="text-h6 mt-4">等待上传文件</div>
              <div class="text-body-1 mt-2">上传发票文件后，将自动进行 OCR 识别</div>
            </div>
          </v-card-text>
        </v-card>
      </v-col>
    ...
    <!-- OCR 状态提示 -->
    <v-alert v-if="ocrStatus" :type="ocrStatus.available ? 'success' : 'warning'" class="mt-4">
      ...
      <div v-else>
        ...
        <v-btn variant="text" size="small" @click="checkOcrStatus" class="ml-2">
          重新检查
        </v-btn>
      </div>
    </v-alert>
...
const onFileSelect = (event) => {
  ...
  console.log('文件选择事件:', files.value?.length, '个文件')
  if (files.value?.length) {
    console.log('文件名:', files.value.map(f => f.name))
  }
}
...
const uploadFiles = async () => {
  console.log('uploadFiles 被调用!', '文件数量:', files.value?.length, 'files:', files.value)
        ...
        for (const file of files.value) {
          ...
          if (autoCreateInvoice.value) {
            try {
              ...
              const importResult = await ocrApi.importInvoice(invoiceData)
              console.log(`自动导入成功: ${importResult.invoice_number}`)
              // 从列表中移除已自动导入的项
