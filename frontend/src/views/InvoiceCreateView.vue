        ...
        <v-tabs-window-item value="ocr">
          <v-row>
            ...
            <v-col cols="12" md="5">
              <v-card variant="outlined">
                <v-card-text class="pa-6">
                  <div class="text-h6 mb-4">📎 上传发票文件</div>
                  ...
                  <v-expansion-panels variant="accordion" class="mb-4">
                    <v-expansion-panel>
                      <v-expansion-panel-title>
                        <v-icon left size="small" class="mr-2">mdi-tune</v-icon>
                        OCR 识别设置
                      </v-expansion-panel-title>
            ...
            <v-col cols="12" md="7">
              <v-card variant="outlined">
                <v-card-text>
                  <div class="text-h6 mb-4">📋 识别结果</div>

                  <!-- 识别中 -->
                  <div v-if="ocrProcessing" class="text-center py-8">
                    <v-progress-circular indeterminate color="primary" size="48"></v-progress-circular>
                    <div class="mt-3 text-body-1">正在识别中<span class="dots">...</span></div>
                  </div>

                  <!-- 结果列表 -->
                  <div v-else-if="ocrResults.length > 0">
                    <v-expansion-panels v-model="expandedResults" multiple>
                      <v-expansion-panel
                      ...
                      >
                        <v-expansion-panel-title>
                          <template v-slot:default="{ expanded }">
                            <v-list-item class="pa-0" :class="{ 'bg-grey-lighten-4': expanded }">
                              <template v-slot:prepend>
                                <v-icon :color="item.imported ? 'success' : 'primary'" size="20">
                              ...
                              <v-list-item-subtitle class="text-caption" v-if="!item.imported">
                        ...
                        <v-expansion-panel-text v-if="!item.imported">
                          ...
                          <v-table density="compact" class="mb-3">
                            <tbody>
                              <tr>
                                <td class="font-weight-bold text-caption" width="100">发票号码</td>
                                <td class="text-body-2">{{ item.data?.invoice_number || '-' }}</td>
                              </tr>
                              <tr>
                                <td class="font-weight-bold text-caption">发票代码</td>
                                <td class="text-body-2">{{ item.data?.invoice_code || '-' }}</td>
                              </tr>
                              <tr>
                                <td class="font-weight-bold text-caption">发票类型</td>
                                <td class="text-body-2">{{ item.data?.invoice_type || '-' }}</td>
                              </tr>
                              <tr>
                                <td class="font-weight-bold text-caption">开票日期</td>
                                <td class="text-body-2">{{ item.data?.invoice_date || '-' }}</td>
                              </tr>
                              <tr>
                                <td class="font-weight-bold text-caption">含税金额</td>
                                <td class="text-body-2 font-weight-bold">
                                  ¥{{ formatAmount(item.data?.total_with_tax) }}
                                </td>
                              </tr>
                              <tr>
                                <td class="font-weight-bold text-caption">税额</td>
                                <td class="text-body-2">¥{{ formatAmount(item.data?.tax_amount) }}</td>
                              </tr>
                              <tr>
                                <td class="font-weight-bold text-caption">销方单位</td>
                                <td class="text-body-2">{{ item.counterpartName || '-' }}</td>
                              </tr>
                            </tbody>
                          </v-table>
                  ...
                  <div v-else class="text-center py-8">
                    ...
                    <div class="text-body-1 mt-3 text-grey">上传发票文件后将自动进行 OCR 识别</div>
                    <div class="text-caption text-grey mt-1">或切换到「手动录入」标签直接填表</div>
                  </div>
                </v-card-text>
              </v-card>
            </v-col>
        ...
        <v-tabs-window-item value="manual">
          <v-form ref="manualFormRef" @submit.prevent="submitManual">
            ...
            <div class="d-flex align-center mb-3">
              ...
              <v-spacer></v-spacer>
              <v-btn color="secondary" size="small" variant="tonal" @click="addManualDetail">
            ...
            >
              <v-row dense>
                ...
                <v-col cols="6" md="1">
                  <v-btn icon variant="text" size="small" color="error" @click="removeManualDetail(index)">
            ...
            <div v-if="manualForm.details.length === 0" class="text-center py-3">
