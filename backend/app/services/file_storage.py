...
def store_file(file_content: bytes, file_name: str, invoice_id: int) -> Tuple[str, Optional[str], Optional[str]]:
    """
    存储文件，根据大小选择存储方式
...
def retrieve_file(storage_mode: str, file_path: Optional[str], blob_data: Optional[str]) -> bytes:
    """根据存储方式读取文件内容"""
...
def delete_file(storage_mode: str, file_path: Optional[str]) -> None:
    """删除文件（仅对 path 模式有效）"""
